...
const SinglePost = () => {
  ...
  useEffect(() => {
    fetch(`https://db.tadeasfort.com/api/articles/${id}?populate=*`)
      ...
      <VStack spacing={4} align="start">
        <Box position="relative">
          <ChakraImage
            boxSize="100%"
            objectFit="cover"
            src={`https://db.tadeasfort.com/${article.attributes.hero.data.attributes.formats.large.url}`}
        ...
        <Gallery>
          ...
          >
            {article.attributes.gallery?.data?.map((item, index) => {
              const imageUrl = `https://db.tadeasfort.com/${item.attributes.formats.large.url}`;
              return (
                <Item
                  key={index}
                  original={imageUrl}
